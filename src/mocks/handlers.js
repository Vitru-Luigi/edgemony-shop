...
const savedData = localStorageJSON ? JSON.parse(localStorageJSON) : {orders: [], carts: [], cartId: 0, v: '0.1'};
...

{
	...
	localStorage.setItem('edgemony-cart', JSON.stringify(getCartWithProducts(cart)));
}
...
export const handlers = [
	rest.get(
		...
		randomError(function getProductsApi(req, res, ctx) {
			...
			if (q) {
				filteredProducts = filteredProducts.filter((product) => product.title.search(new RegExp(q, 'i')) !== -1);
			}
			if (limit) {
	...
	rest.post(
		...
		randomError(function addToCartApi(req, res, ctx) {
			try {
				...
				return res(ctx.status(200), ctx.json(addToCart(id, productId, quantity)));
			} catch (error) {
				if (error instanceof HttpError) {
					return res(ctx.status(error.status), ctx.json({message: error.message}));
				}
